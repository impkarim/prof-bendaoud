...
function renderNavbar() {
  ...
  const navLinks = data.links
    .map((link) => {
      const href = link.href.startsWith("#") ? `${basePath}index.html${link.href}` : `${basePath}${link.href}`;
...
function getCategory(description, title) {
  ...
  if (t.includes("استاذ") || t.includes("profess") || t.includes("maître") || t.includes("assistant") || t.includes("chargé") || t.includes("lectur") || t.includes("مندوب") || t.includes("محاضر") || t.includes("مكلف") || t.includes("doyen") || t.includes("عميد") || t.includes("dean") || t.includes("رئيس قسم") || t.includes("head of dep") || t.includes("chef")) return { label: { ar: "تدرج مهني", en: "Promotion", fr: "Promotion" }, color: "from-amber-400/20 to-amber-600/10 border-amber-500/20", badgeBg: "bg-amber-500/10" };
  if (t.includes("تدريس") || t.includes("تعليم") || t.includes("تكوين") || t.includes("formation") || t.includes("training") || t.includes("course") || t.includes("دورة") || t.includes("تدري") || t.includes("teach") || t.includes("supervis")) return { label: { ar: "تدريس", en: "Teaching", fr: "Enseignement" }, color: "from-blue-400/20 to-blue-600/10 border-blue-500/20", badgeBg: "bg-blue-500/10" };
  if (t.includes("بحث") || t.includes("recherche") || t.includes("research") || t.includes("cnepru") || t.includes("pnr") || t.includes("prfu") || t.includes("فرقة") || t.includes("مشروع") || t.includes("project") || t.includes("groupe") || t.includes("équipe") || t.includes("team") || t.includes("laborat") || t.includes("مخبر")) return { label: { ar: "بحث علمي", en: "Research", fr: "Recherche" }, color: "from-emerald-400/20 to-emerald-600/10 border-emerald-500/20", badgeBg: "bg-emerald-500/10" };
  if (t.includes("مجلة") || t.includes("revue") || t.includes("journal") || t.includes("تحرير") || t.includes("rédact") || t.includes("editor") || t.includes("تحكيم") || t.includes("arbitrag") || t.includes("نشر") || t.includes("public") || t.includes("مؤسس") || t.includes("fondat") || t.includes("founder") || t.includes("مدير")) return { label: { ar: "تحرير ونشر", en: "Editorial", fr: "Rédaction" }, color: "from-purple-400/20 to-purple-600/10 border-purple-500/20", badgeBg: "bg-purple-500/10" };
  if (t.includes("ملتقى") || t.includes("ندوة") || t.includes("مؤتمر") || t.includes("forum") || t.includes("conférence") || t.includes("conference") || t.includes("symposium") || t.includes("colloque") || t.includes(" يوم ") || t.includes("journée") || t.includes("study day")) return { label: { ar: "ملتقيات", en: "Conferences", fr: "Colloques" }, color: "from-rose-400/20 to-rose-600/10 border-rose-500/20", badgeBg: "bg-rose-500/10" };
  if (t.includes("لجنة") || t.includes("comité") || t.includes("committee") || t.includes("commission") || t.includes("مجلس") || t.includes("conseil") || t.includes("council") || t.includes("quality") || t.includes("جودة") || t.includes("assurance") || t.includes("عضوية") || t.includes("membre") || t.includes("member")) return { label: { ar: "عضوية لجان", en: "Committee", fr: "Comité" }, color: "from-cyan-400/20 to-cyan-600/10 border-cyan-500/20", badgeBg: "bg-cyan-500/10" };
  return { label: { ar: "إنجاز", en: "Achievement", fr: "Réalisation" }, color: "from-amber-400/20 to-amber-600/10 border-amber-500/20", badgeBg: "bg-amber-500/10" };
}
...
function renderContent() {
  ...
  icon.className = lang === "ar" ? "fas fa-arrow-right ml-1" : "fas fa-arrow-left mr-1";
}
